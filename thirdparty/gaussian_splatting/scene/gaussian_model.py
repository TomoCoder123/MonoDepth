...
class GaussianModel:
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            ...
            depth,
            depth_scale=1.0,
            depth_trunc=self.config["mapping"]["pcd_depth_trunc"],
    ...
    def extend_from_pcd(
        self, fused_point_cloud, features, scales, rots, opacities, kf_id, labels=None
    ):
        ...
        if labels is None:
            new_labels = torch.zeros((new_xyz.shape[0])).int()
        else:
            new_labels = labels
    ...
    def extend_from_pcd_seq(
        self, cam_info, kf_id=-1, init=False, scale=2.0, depthmap=None, labels=None
    ):
        fused_point_cloud, features, scales, rots, opacities = self.create_pcd_from_image(
            cam_info, init, scale=scale, depthmap=depthmap
        )
        self.extend_from_pcd(fused_point_cloud, features, scales, rots, opacities, kf_id, labels)
    ...
    def load_ply(self, path):
        ...
        self.labels = torch.zeros((self._xyz.shape[0]), device="cpu").int()
    ...
    def prune_points(self, mask):
        ...
        self.labels = self.labels[valid_points_mask.cpu()]
    ...
    def densification_postfix(
        ...
        if new_labels is not None:
            self.labels = torch.cat((self.labels, new_labels)).int()
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_labels = self.labels[selected_pts_mask.cpu()].repeat(N)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_labels = self.labels[selected_pts_mask.cpu()]
        self.densification_postfix(
            new_xyz,
            new_features_dc,
            new_features_rest,
